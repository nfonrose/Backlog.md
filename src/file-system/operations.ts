...
export class FileSystem {
	...
	/**
	 * Loads centralized task ordinals from task-ordinals.yml
	 */
	public async loadTaskOrdinals(): Promise<Record<string, number>> {
		try {
			const ordinalsPath = join(this.resolvedBacklogDir, DEFAULT_FILES.TASK_ORDINALS);
			const file = Bun.file(ordinalsPath);
			...
			for (const line of lines) {
				...
				const id = trimmed.substring(0, colonIndex).trim().toUpperCase();
				const value = Number.parseFloat(trimmed.substring(colonIndex + 1).trim());
	...
	/**
	 * Saves centralized task ordinals to task-ordinals.yml, sorted by ID
	 */
	public async saveTaskOrdinals(ordinals: Record<string, number>): Promise<void> {
		const ordinalsPath = join(this.resolvedBacklogDir, DEFAULT_FILES.TASK_ORDINALS);
		const sortedIds = Object.keys(ordinals).sort();
		const lines = sortedIds.map((id) => `${id.toUpperCase()}: ${ordinals[id]}`);
		const content = `${lines.join("\n")}\n`;
		await this.ensureDirectoryExists(this.resolvedBacklogDir);
		await Bun.write(ordinalsPath, content);
	}

	/**
	 * Migrates ordinals from individual task files to the central task-ordinals.yml file.
	 * Only proceeds if centralized_tasks_ordinals is enabled in config.
	 */
	public async migrateOrdinalsToCentralStorage(): Promise<{ migratedCount: number }> {
		const config = await this.loadConfig();
		if (!config.centralizedTasksOrdinals) {
			return { migratedCount: 0 };
		}

		// We need to list tasks without applying centralized ordinals to get what's in the files
		// But our listTasks already applies them if enabled.
		// Let's implement a direct scan.
		const activeDir = await this.getTasksDir();
		...
		const taskFiles = [
			...(await Array.fromAsync(new Bun.Glob(globPattern).scan({ cwd: activeDir, followSymlinks: true }))).map((f) =>
				join(activeDir, f),
			),
			...(await Array.fromAsync(new Bun.Glob(globPattern).scan({ cwd: completedDir, followSymlinks: true }))).map((f) =>
				join(completedDir, f),
			),
		];

		const currentOrdinals = await this.loadTaskOrdinals();
		let migratedCount = 0;
		...
		for (const filepath of taskFiles) {
			try {
				...
				if (task.ordinal !== undefined) {
					...
					if (currentOrdinals[id] === undefined) {
						currentOrdinals[id] = task.ordinal;
						migratedCount++;
					}
				}
			} catch (_error) {
		...
		if (migratedCount > 0) {
			await this.saveTaskOrdinals(currentOrdinals);
		}
	...
	async saveTask(task: Task): Promise<string> {
		...
		if (config?.centralizedTasksOrdinals && task.ordinal !== undefined) {
			const ordinals = await this.loadTaskOrdinals();
			ordinals[taskId.toUpperCase()] = task.ordinal;
			await this.saveTaskOrdinals(ordinals);
		}
		...
		const content = serializeTask(normalizedTask, { excludeOrdinal: config?.centralizedTasksOrdinals });
		...
		try {
			...
			} else if (existingPath && existingPath.endsWith(filename)) {
				// If file already exists with correct name, check if content actually changed
				const existingContent = await Bun.file(existingPath).text();
				if (existingContent === content) {
					return existingPath; // No change, skip write
				}
			}
		} catch {
	...
	async loadTask(taskId: string): Promise<Task | null> {
		try {
			...
			if (config?.centralizedTasksOrdinals) {
				...
				if (ordinals[normalizedId] !== undefined) {
					task.ordinal = ordinals[normalizedId];
				}
			}
...
${description || `Milestone: ${title}`}`,
	...
	async loadConfig(): Promise<BacklogConfig | null> {
		...
		try {
			...
			// If centralized ordinals enabled, check if we need to perform initial migration
			if (config.centralizedTasksOrdinals) {
				const ordinalsPath = join(this.resolvedBacklogDir, DEFAULT_FILES.TASK_ORDINALS);
				if (!(await Bun.file(ordinalsPath).exists())) {
					// Async migration in background or wait? Let's wait for the first time
					await this.migrateOrdinalsToCentralStorage();
				}
			}
	...
	private parseConfig(content: string): BacklogConfig {
		...
		for (const line of lines) {
			...
			switch (key) {
				...
				case "sort_done_by_recency":
					...
					break;
				case "centralized_tasks_ordinals":
					config.centralizedTasksOrdinals = value.toLowerCase() === "true";
					break;
		...
		return {
			...
			centralizedTasksOrdinals: config.centralizedTasksOrdinals,
	...
	private serializeConfig(config: BacklogConfig): string {
		...
		const lines = [
			...
			...(typeof config.centralizedTasksOrdinals === "boolean"
				? [`centralized_tasks_ordinals: ${config.centralizedTasksOrdinals}`]
				: []),
